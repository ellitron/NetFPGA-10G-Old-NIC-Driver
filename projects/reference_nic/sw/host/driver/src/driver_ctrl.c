...
static int do_ghost_enable_recv_ack_cb(struct nl_msg *msg, void *arg)
{
    return 0;
}

static void do_ghost_enable(int argc, char *argv[])
{
    struct nl_msg   *msg;
    int err;    
    ...
    err = driver_connect();
    if(err != 0) {
        printf("ERROR: do_ghost_enable(): failed to connect to the driver\n");
        return;
    }

    msg = nlmsg_alloc();
    if(msg == NULL) {
        printf("ERROR: do_ghost_enable(): could not allocate new netlink message\n");
        driver_disconnect();
        return;
    }
    ...
    /* genlmsg_put will fill in the fields of the nlmsghdr and the genlmsghdr. */
    genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, nf10_genl_family, 0, 0,
            NF10_GENL_C_GHOST_ENABLE, NF10_GENL_FAMILY_VERSION);
    ...
    nl_socket_modify_cb(nf10_genl_sock, NL_CB_ACK, NL_CB_CUSTOM, do_ghost_enable_recv_ack_cb, NULL);
...
static int do_ghost_disable_recv_ack_cb(struct nl_msg *msg, void *arg)
{
    return 0;
}

static void do_ghost_disable(int argc, char *argv[])
{
    struct nl_msg   *msg;
    int err;    
    ...
    err = driver_connect();
    if(err != 0) {
        printf("ERROR: do_ghost_disable(): failed to connect to the driver\n");
        return;
    }

    msg = nlmsg_alloc();
    if(msg == NULL) {
        printf("ERROR: do_ghost_disable(): could not allocate new netlink message\n");
        driver_disconnect();
        return;
    }
    ...
    /* genlmsg_put will fill in the fields of the nlmsghdr and the genlmsghdr. */
    genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, nf10_genl_family, 0, 0,
            NF10_GENL_C_GHOST_DISABLE, NF10_GENL_FAMILY_VERSION);
    ...
    nl_socket_modify_cb(nf10_genl_sock, NL_CB_ACK, NL_CB_CUSTOM, do_ghost_disable_recv_ack_cb, NULL);
...
static void do_help(int argc, char *argv[])
{
 ...
 static struct command all_commands[] = {
    ...
    { "ghost_enable", 0, 0, do_ghost_enable },
    { "ghost_disable", 0, 0, do_ghost_disable },
    { "help", 0, 0, do_help },
    { NULL, 0, 0, NULL },
};
